...
export interface IReview {
    ...
    date: Date;
    ratings: {
        food_quality: number;                   // 0-5 stars
        staff_service: number;                  // 0-5 stars
...
const reviewSchema = new Schema<IReview>({
    ...
    ratings: {
        food_quality: { type: Number, required: true, min: 0, max: 5 },
        staff_service: { type: Number, required: true, min: 0, max: 5 },
